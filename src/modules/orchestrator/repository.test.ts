...
describe('migrations/orchestrator/0001_orchestrator.sql', () => {
  ...
  it('re-applies cleanly on an existing database (M0 acceptance)', () => {
    ...
    // already at its latest version and change nothing. Eight files now: 0001's
    // columns, 0002's `permission_denials` (§8.1's `tool_denials` input),
    // 0003's `exit_reason` (§11.2's "why did this turn fail"), 0004's
    // `pre_grants_json` (§2.3's pre-answered gates), 0005's
    // `permission_denied_tools` (which calls were refused, not only how many),
    // 0006's `template_id` (which task template this came from, WO5), 0007's
    // `triggers` table and 0008's `origin` / `trigger_id` (which standing
    // schedule started this, WO8).
    const reopened = openTestStorage(dir.path);
    expect(reopened.setVersions['orchestrator']).toBe(8);
    const columns = (
