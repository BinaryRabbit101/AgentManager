...
```
%AGENTMANAGER_DATA%/roster/            # path from foundation's config; a git repo in its own right
  agents/
...
The roster directory being a standalone git repo (init'd by the installer, never auto-committed)
is what makes "git-versioning the roster" real rather than aspirational.

### 2.2 What goes in SQLite

Foundation's database holds a single roster-adjacent table, and it contains **no definition
fields**:
...
```jsonc
{
  ...
  "capabilities": {
    ...
    "roles": ["implementer", "skeptic"]     // collaboration slots this agent can fill
  },
...
→ EffectivePermissions` — with a table-driven test suite. It is the highest-risk logic in the
element.
...
```ts
plugins: [{ type: "local", path: "<rosterDir>/agents/<id>" }]
```
...
`{ "secretRef": "<key>" }`. Roster stores only the ref. At session launch the compiler asks
...
`mcp__agentmanager__*` allow rules.
...
compileSession(input: {
  agent: ResolvedAgent;
  project?: ProjectContext;          // cwd, per-project overrides, elevation
  assignment?: AssignmentContext;    // id, role, scope rules, budget
...
|---|---|
| `persona` + role addendum + runtime block | `systemPrompt` (§5) |
...
| foundation | `env` (spread base env, plus `CLAUDE_CODE_DISABLE_AUTO_MEMORY`, `CLAUDE_CONFIG_DIR`) |
