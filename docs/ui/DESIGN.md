...
| Model | primary and fallback are `<select>`s over roster's aliases (`default`/`best`/`opus`/`sonnet`/`haiku`/`opusplan`) with a final **Custom model id…** option that reveals a text input, so a full `claude-*` id stays typeable — roster §8 validates aliases *warn-not-block* precisely so a model released after this build ships cannot make an agent unloadable, and a closed dropdown would throw that away. A stored non-alias loads into the custom input. Effort is a closed `<select>` over `low`/`medium`/`high`/`xhigh`/`max` with **no** escape: that one *is* a hard enum server-side, so a typo is a 400 nobody could have predicted, and the honest fix is to make the wrong value unspellable. The empty choice on each means roster's own default rather than a value |
| Permissions | three rule lists (allow/deny/ask), mode picker, and a plain-language note that deny wins and allow is only auto-approval. Each bucket is **removable chips** in monospace plus one **Add rule** button opening a shared picker with three ways in *(2026-08-19, WO2 — the owner's "knowing what to write for allow, deny, ask is nearly impossible")*: the **catalogue** from `GET /api/roster/permission-catalogue`, grouped, each entry showing its rule and its plain-language description, with a soft hint when the entry's `suggest` differs from the open bucket; **Compose**, a tool `<select>` (roster's nine named tools plus an `mcp__<server>__*` entry per integration **currently in the form**, since a server being added right now has no route to offer it) and an optional pattern, producing `Tool(pattern)` or a bare `Tool`; and **Raw rule**, an always-present escape hatch, because roster's grammar is wider than any picker. The bucket help lines say what each list *does* — allow auto-approves, deny always blocks and wins over everything, ask makes a human answer a card. Client-side **warnings never gate** (roster stays the authority) and mirror `sdkRules.ts`'s own prose: a `Write(path)`/`NotebookEdit(path)` rule is stored as `Edit(path)`, an `allow` on `AskUserQuestion` is lifted into `ask`, `Edit(*)` collapses to auto-approving every edit, an unbalanced parenthesis, a duplicate in the same bucket. A failed catalogue fetch degrades the picker to Compose + Raw and nothing else — the form is never unusable for want of a suggestion |
| Roles | checkboxes over the pinned five — which seats this agent may be given |
| Role addenda | a markdown textarea per role, holding `roles/<role>.md` verbatim (roster §4) — how it behaves *once it is in* a seat. Collapsed behind a disclosure, "Role addenda — optional, for team seats", closed unless the agent already has one, and led by the two sentences that make the feature legible on screen: an addendum is appended **only** when the orchestrator seats this agent in that collaboration role, so solo runs never read one and most agents need none. A box is rendered for a role that is checked in **Roles** above or already has content; the remaining roles are reached through an **Add addendum for…** picker, because §4 keeps the seat list and the addenda independent — a role may carry an addendum without being listed, and an unlisted one is still labelled as such. Emptying a box deletes the file; a box revealed and never typed in writes nothing. Drafting never fills these in |
...
| **Rich agent-permission editor** (pattern autocomplete, per-rule dry-run against a project) | **Partly landed** *(2026-08-19, WO2)*: `GET /api/roster/permission-catalogue` exposed the catalogue this row said the API did not, so §7.1's Permissions row is now chips plus a catalogue/compose/raw picker with client-side warnings. What is still deferred is the half that needs more than a list — completing a glob against a real workspace, and previewing one rule's effect without saving. |
